...
const translations = {
    ky: {
        ...
        comments: "Изоҳ ёзилган", businessTools: "Бизнес куралдары", goals: "Мақсаттар",
        calculator: "Калькулятор", projects: "Долбоорлор"
    },
    en: {
        ...
        editProfile: "Edit profile", saves: "Saved", likes: "Liked", comments: "Commented",
        businessTools: "Business tools", goals: "Goals", calculator: "Calculator", projects: "Projects"
    },
    ru: {
        ...
        comments: "Прокомментированные", businessTools: "Бизнес-инструменты", goals: "Цели",
        calculator: "Калькулятор", projects: "Проекты"
    },
    tr: {
        ...
        comments: "Yorum yapılanlar", businessTools: "İş araçları", goals: "Hedefler",
        calculator: "Hesap makinesi", projects: "Projeler"
    },
    ar: {
        ...
        comments: "التعليقات", businessTools: "أدوات الأعمال", goals: "الأهداف",
        calculator: "آلة حاسبة", projects: "المشاريع"
    }
};
...
function setTheme(theme) {
    try {
        ...
        console.log("Tema o‘zgartirildi:", theme);
    } catch(e) { console.error("Tema xatosi:", e); }
}
// Tugmalar uchun global qilish
...
function renderPosts(posts) {
    ...
    posts.forEach(post => {
        ...
        const canEdit = currentUser && (currentUser.uid === post.userId || (window.ADMIN_EMAILS && window.ADMIN_EMAILS.includes(currentUser.email)));
        html += `
...
async function addComment() {
    ...
    await db.collection('posts').doc(currentCommentPostId).update({
...
// ================= PROFIL RASMI (BASE64) VA EMOJI =================
// Base64 orqali rasm yuklash (Firebase Storage kerak emas)
async function updateProfilePhotoBase64() {
    const file = document.getElementById('profilePhotoInput')?.files[0];
    if (!file) return;
    if (file.size > 500 * 1024) {
        alert("Rasm hajmi 500 KB dan kichik bo‘lishi kerak!");
        return;
    }
    ...
    reader.onload = async function(e) {
        ...
        await usersRef.doc(currentUser.uid).set({
            photoBase64: base64,
            profileEmoji: null  // agar rasm yuklansa, emoji o‘chadi
        }, { merge: true });
        alert("Rasm saqlandi!");
        showProfilePage();
    };
...
window.updateProfilePhotoBase64 = updateProfilePhotoBase64;
...
function showEmojiPicker() {
    ...
    let html = `<div style="display: grid; grid-template-columns: repeat(6, 1fr); gap: 10px; max-height: 300px; overflow-y: auto; padding: 5px;">`;
    emojis.forEach(emoji => {
        html += `<div style="font-size: 2rem; text-align: center; cursor: pointer; padding: 5px; background: rgba(255,255,255,0.1); border-radius: 12px;" onclick="selectEmoji('${emoji}')">${emoji}</div>`;
    });
    html += `</div><div style="margin-top: 15px;"><input type="text" id="customEmoji" placeholder="Yoki o‘zingiz emoji yozing (masalan: 😎)" class="input-glass"><button class="btn-gradient" style="margin-top: 8px;" onclick="selectCustomEmoji()">Tanlash</button></div>`;
    showModal(html, "Emoji tanlang");
}
window.showEmojiPicker = showEmojiPicker;

function selectEmoji(emoji) {
    saveProfileEmoji(emoji);
    closeModal();
}
window.selectEmoji = selectEmoji;

function selectCustomEmoji() {
    let emoji = document.getElementById('customEmoji')?.value.trim();
    if (emoji) {
        saveProfileEmoji(emoji);
        closeModal();
    } else {
...
async function saveProfileEmoji(emoji) {
    ...
    alert(`✅ Profil emojisi "${emoji}" saqlandi!`);
    showProfilePage();
}
window.saveProfileEmoji = saveProfileEmoji;
...
// ================= PROFIL SAHIFASI (EMOJI / BASE64 / INITIALS) =================
async function showProfilePage() {
    ...
    let html = `
        <div class="post-card" style="text-align:center">
            ...
            <div style="margin-top: 10px; display: flex; gap: 8px; flex-wrap: wrap; justify-content: center;">
                ...
                <button class="btn-gradient" onclick="openAdminModal()">➕ ${translations[currentLang].addPost}</button>
                <button class="btn-gradient" onclick="logout()" style="background:#555">🚪 ${translations[currentLang].logout}</button>
            </div>
            <div style="margin-top:20px">
                <button class="btn-gradient" onclick="showBusinessTools()" style="background: linear-gradient(135deg, #f39c12, #e67e22)"><i class="fas fa-chart-line"></i> ${translations[currentLang].businessTools}</button>
            </div>
    ...
    if (fileInput) fileInput.addEventListener('change', updateProfilePhotoBase64);
}
...
function showBusinessTools() {
    ...
    let html = `<h3><i class="fas fa-chart-line"></i> ${t.businessTools}</h3>
...
window.showBusinessTools = showBusinessTools;
...
function showGoals() {
    ...
    let html = `<div><input id="newGoal" class="input-glass" placeholder="Янги мақсад"><button class="btn-gradient" onclick="addGoal()">Қўшиш</button><ul id="goalsList">` +
        goals.map((g, i) => `<li style="margin:8px 0">✅ ${g} <button onclick="removeGoal(${i})">Өчүрүү</button></li>`).join('') +
        `</ul></div>`;
...
function showCalculator() {
    let html = `<div><input id="income" class="input-glass" placeholder="Кирим (сом)"><input id="expense" class="input-glass" placeholder="Чыгым (сом)"><button class="btn-gradient" onclick="calculateProfit()">Ҳисоблаш</button><p id="calcResult" style="margin-top:15px"></p></div>`;
...
window.calculateProfit = function() {
    ...
    if (result) result.innerHTML = `💰 Соф фойда: ${profit} сом`;
};
...
function showProjects() {
    ...
    let html = `<div><input id="newProject" class="input-glass" placeholder="Лойиҳа номи"><button class="btn-gradient" onclick="addProject()">Қўшиш</button><ul id="projectsList">` +
        projects.map((p, i) => `<li style="margin:8px 0">📌 ${p} <button onclick="removeProject(${i})">Өчүрүү</button></li>`).join('') +
        `</ul></div>`;
...
function openAdminModal() {
    if (!currentUser) { openLoginModal(); return; }
    if (!window.ADMIN_EMAILS || !window.ADMIN_EMAILS.includes(currentUser.email)) {
        alert("Администратор эмессиз!");
        return;
    }
...
function setupBottomNav() {
    ...
    if (!navItems.length) {
        console.error("Bottom nav elementlari topilmadi!");
        return;
    }
...
// ================= QO‘SHIMCHA KICHIK FUNKSIYALAR =================
function showTasbeeh() {
    let html = `<div style="text-align:center"><div id="tasbCount" style="font-size:3rem">0</div><button class="btn-gradient" onclick="document.getElementById('tasbCount').innerText = (parseInt(document.getElementById('tasbCount').innerText)||0)+1">+1</button><button onclick="document.getElementById('tasbCount').innerText = 0">Reset</button></div>`;
...
// ================= INITIALIZATION (BOSHLANG‘ICH) =================
document.addEventListener('DOMContentLoaded', () => {
    // Tugmalar uchun event listenerlar
    const submitBtn = document.getElementById('submitPostBtn');
    if (submitBtn) submitBtn.onclick = addNewPost;
    ...
    // Tema tugmalariga ishonch hosil qilish
    const moonIcon = document.querySelector('.fa-moon')?.parentElement;
    const sunIcon = document.querySelector('.fa-sun')?.parentElement;
    const bookIcon = document.querySelector('.fa-book-open')?.parentElement;
    if (moonIcon) moonIcon.setAttribute('onclick', "setTheme('dark')");
    if (sunIcon) sunIcon.setAttribute('onclick', "setTheme('light')");
    if (bookIcon) bookIcon.setAttribute('onclick', "setTheme('reading')");
});