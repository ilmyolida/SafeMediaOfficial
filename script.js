...
const photoBase64 = userData.photoBase64 || 'https://via.placeholder.com/80';
html += `<img src="${photoBase64}" class="profile-avatar">`;
// ================= FIREBASE REFS =================
const usersRef = db.collection('users');

// ================= TILLAR (to‘liq) =================
const translations = {
    ky: { home: "Башкы", info: "Маалымат", community: "Жамият", profile: "Профиль", delete: "Өчүрүү", edit: "Таҳрирлаш", like: "Лайк", save: "Сактоо", comment: "Изоҳ", noPosts: "Посттор жок", loginRequired: "Кирүү керек", loginDesc: "Google менен кириңиз", loginWithGoogle: "Google менен кирүү", logout: "Чыгуу", addPost: "Пост кошуу", settings: "Орнотуулар", clearCache: "Кешни тазалоо", exportData: "Маалыматтарды жүктөө", editProfile: "Профилди оңдоо", saves: "Сакталган", likes: "Лайк басылган", comments: "Изоҳ ёзилган", businessTools: "Бизнес куралдары", goals: "Мақсаттар", calculator: "Калькулятор", projects: "Долбоорлор" },
    en: { home: "Home", info: "Info", community: "Community", profile: "Profile", delete: "Delete", edit: "Edit", like: "Like", save: "Save", comment: "Comment", noPosts: "No posts", loginRequired: "Login required", loginDesc: "Login with Google", loginWithGoogle: "Login with Google", logout: "Logout", addPost: "Add post", settings: "Settings", clearCache: "Clear cache", exportData: "Export data", editProfile: "Edit profile", saves: "Saved", likes: "Liked", comments: "Commented", businessTools: "Business tools", goals: "Goals", calculator: "Calculator", projects: "Projects" },
    ru: { home: "Главная", info: "Инфо", community: "Сообщество", profile: "Профиль", delete: "Удалить", edit: "Редактировать", like: "Нравится", save: "Сохранить", comment: "Комментарий", noPosts: "Нет постов", loginRequired: "Требуется вход", loginDesc: "Войдите через Google", loginWithGoogle: "Войти через Google", logout: "Выйти", addPost: "Добавить пост", settings: "Настройки", clearCache: "Очистить кэш", exportData: "Экспорт данных", editProfile: "Редактировать профиль", saves: "Сохранённые", likes: "Понравившиеся", comments: "Прокомментированные", businessTools: "Бизнес-инструменты", goals: "Цели", calculator: "Калькулятор", projects: "Проекты" },
    tr: { home: "Ana Sayfa", info: "Bilgi", community: "Topluluk", profile: "Profil", delete: "Sil", edit: "Düzenle", like: "Beğen", save: "Kaydet", comment: "Yorum", noPosts: "Gönderi yok", loginRequired: "Giriş gerekli", loginDesc: "Google ile giriş yapın", loginWithGoogle: "Google ile giriş", logout: "Çıkış", addPost: "Gönderi ekle", settings: "Ayarlar", clearCache: "Önbelleği temizle", exportData: "Verileri dışa aktar", editProfile: "Profili düzenle", saves: "Kaydedilenler", likes: "Beğenilenler", comments: "Yorum yapılanlar", businessTools: "İş araçları", goals: "Hedefler", calculator: "Hesap makinesi", projects: "Projeler" },
    ar: { home: "الرئيسية", info: "معلومات", community: "المجتمع", profile: "الملف الشخصي", delete: "حذف", edit: "تعديل", like: "إعجاب", save: "حفظ", comment: "تعليق", noPosts: "لا توجد منشورات", loginRequired: "تسجيل الدخول مطلوب", loginDesc: "تسجيل الدخول مع Google", loginWithGoogle: "تسجيل الدخول مع Google", logout: "تسجيل الخروج", addPost: "إضافة منشور", settings: "الإعدادات", clearCache: "مسح ذاكرة التخزين المؤقت", exportData: "تصدير البيانات", editProfile: "تعديل الملف الشخصي", saves: "المحفوظات", likes: "الإعجابات", comments: "التعليقات", businessTools: "أدوات الأعمال", goals: "الأهداف", calculator: "آلة حاسبة", projects: "المشاريع" }
};
...
async function addComment() {
    ...
    await db.collection('posts').doc(currentCommentPostId).update({ comments: firebase.firestore.FieldValue.arrayUnion(comment) });
    document.getElementById('newComment').value = '';
    closeCommentModal();
    loadPosts();
}

// ================= PROFIL SURATI VA ISM O‘ZGARTIRISH =================
...
async function updateProfilePhoto() {
    ...
    if (!file) return;
    // Yuklash indikatori
    const btn = document.querySelector('#profilePhotoInput + button');
    if (btn) btn.disabled = true;
    const storageRef = storage.ref().child(`profile_photos/${currentUser.uid}`);
    const uploadTask = storageRef.put(file);
    uploadTask.on('state_changed', null, (error) => { console.error(error); alert('Хато!'); if(btn) btn.disabled = false; },
        async () => {
...
// ================= PROFIL SAHIFASI (Like, Save, Commented) =================
async function showProfilePage() {
    if (!currentUser) {
        postsContainer.innerHTML = `<div class="post-card" style="text-align:center"><i class="fas fa-user-circle" style="font-size:4rem"></i><h3>${translations[currentLang].loginRequired}</h3><p>${translations[currentLang].loginDesc}</p><button class="btn-gradient" onclick="openLoginModal()"><i class="fab fa-google"></i> ${translations[currentLang].loginWithGoogle}</button></div>`;
        return;
    }
    ...
    // Like qilingan, saqlangan, izoh qoldirilgan postlar
    ...
    const commentedPosts = allPosts.filter(p => p.comments?.some(c => c.userId === currentUser.uid));
    ...
    let html = `
        <div class="post-card" style="text-align:center">
            ...
            <div style="margin-top: 10px;">
                ...
                <button class="btn-gradient" onclick="document.getElementById('profilePhotoInput').click()" style="background:#555; width:auto; display:inline-block; padding:8px 16px"><i class="fas fa-camera"></i> Сүрөт жүктөө</button>
            ...
            <div style="margin-top:20px">
                <button class="btn-gradient" onclick="showBusinessTools()" style="background: linear-gradient(135deg, #f39c12, #e67e22); margin-bottom:10px"><i class="fas fa-chart-line"></i> ${translations[currentLang].businessTools}</button>
            </div>
            <div class="profile-liked-section"><h4><i class="fas fa-heart"></i> ${translations[currentLang].likes} (${likedPosts.length})</h4>${renderMiniPosts(likedPosts)}</div>
            <div class="profile-saved-section"><h4><i class="fas fa-bookmark"></i> ${translations[currentLang].saves} (${savedPosts.length})</h4>${renderMiniPosts(savedPosts)}</div>
            <div class="profile-commented-section"><h4><i class="fas fa-comment-dots"></i> ${translations[currentLang].comments} (${commentedPosts.length})</h4>${renderMiniPosts(commentedPosts)}</div>
...
function renderMiniPosts(posts) {
    ...
    posts.forEach(p => {
        html += `<div class="post-mini"><strong>${escapeHtml(p.title)}</strong><p style="font-size:0.8rem; margin-top:5px">${escapeHtml(p.content.substring(0,80))}...</p></div>`;
    });
    html += '</div>';
    return html;
}
function exportUserData() {
    const data = {
        ...
        commentedPosts: allPosts.filter(p => p.comments?.some(c => c.userId === currentUser.uid))
    };
...
function showBusinessTools() {
    ...
    let html = `<h3><i class="fas fa-chart-line"></i> ${t.businessTools}</h3><button class="btn-gradient" onclick="showGoals()" style="margin-bottom:10px"><i class="fas fa-bullseye"></i> ${t.goals}</button><button class="btn-gradient" onclick="showCalculator()" style="margin-bottom:10px"><i class="fas fa-calculator"></i> ${t.calculator}</button><button class="btn-gradient" onclick="showProjects()"><i class="fas fa-project-diagram"></i> ${t.projects}</button>`;
    showModal(html, t.businessTools);
}
function showGoals() { let goals = JSON.parse(localStorage.getItem('businessGoals') || '[]'); let html = `<div><input id="newGoal" class="input-glass" placeholder="Янги мақсад"><button class="btn-gradient" onclick="addGoal()">Қўшиш</button><ul id="goalsList">` + goals.map((g, i) => `<li style="margin:8px 0">✅ ${g} <button onclick="removeGoal(${i})">Өчүрүү</button></li>`).join('') + `</ul></div>`; showModal(html, "Мақсадлар"); }
window.addGoal = function() { let input = document.getElementById('newGoal'); if (input.value) { let goals = JSON.parse(localStorage.getItem('businessGoals') || '[]'); goals.push(input.value); localStorage.setItem('businessGoals', JSON.stringify(goals)); showGoals(); } };
window.removeGoal = function(i) { let goals = JSON.parse(localStorage.getItem('businessGoals') || '[]'); goals.splice(i, 1); localStorage.setItem('businessGoals', JSON.stringify(goals)); showGoals(); };
function showCalculator() { let html = `<div><input id="income" class="input-glass" placeholder="Кирим (сом)"><input id="expense" class="input-glass" placeholder="Чыгым (сом)"><button class="btn-gradient" onclick="calculateProfit()">Ҳисоблаш</button><p id="calcResult" style="margin-top:15px"></p></div>`; showModal(html, "Калькулятор"); }
window.calculateProfit = function() { let income = parseFloat(document.getElementById('income').value) || 0; let expense = parseFloat(document.getElementById('expense').value) || 0; let profit = income - expense; document.getElementById('calcResult').innerHTML = `💰 Соф фойда: ${profit} сом`; };
function showProjects() { let projects = JSON.parse(localStorage.getItem('projects') || '[]'); let html = `<div><input id="newProject" class="input-glass" placeholder="Лойиҳа номи"><button class="btn-gradient" onclick="addProject()">Қўшиш</button><ul id="projectsList">` + projects.map((p, i) => `<li style="margin:8px 0">📌 ${p} <button onclick="removeProject(${i})">Өчүрүү</button></li>`).join('') + `</ul></div>`; showModal(html, "Лойиҳалар"); }
window.addProject = function() { let input = document.getElementById('newProject'); if (input.value) { let projects = JSON.parse(localStorage.getItem('projects') || '[]'); projects.push(input.value); localStorage.setItem('projects', JSON.stringify(projects)); showProjects(); } };
window.removeProject = function(i) { let projects = JSON.parse(localStorage.getItem('projects') || '[]'); projects.splice(i, 1); localStorage.setItem('projects', JSON.stringify(projects)); showProjects(); };
...
async function addNewPost() { const title = document.getElementById('postTitle').value.trim(); const content = document.getElementById('postContent').value.trim(); const category = document.getElementById('postCategory').value; if (!title || !content) { alert("Сарлавҳа ва мазмунни тўлдиринг!"); return; } await db.collection('posts').add({ title, content, category, userId: currentUser.uid, userName: currentUser.displayName, timestamp: firebase.firestore.FieldValue.serverTimestamp(), likes: [], saves: [], comments: [] }); closeAdminModal(); loadPosts(); }
...
async function saveEdit() { const title = document.getElementById('editTitle').value.trim(); const content = document.getElementById('editContent').value.trim(); const category = document.getElementById('editCategory').value; if (!title || !content) return; await db.collection('posts').doc(currentEditPostId).update({ title, content, category }); closeEditModal(); loadPosts(); }
...
// ================= SEARCH =================
function searchPosts() { const term = searchInput.value.toLowerCase(); if (!term) { filterAndRender(); return; } const filtered = allPosts.filter(p => p.title.toLowerCase().includes(term) || p.content.toLowerCase().includes(term)); renderPosts(filtered); }

// ================= BOTTOM NAV =================
function setupBottomNav() { document.querySelectorAll('.nav-item').forEach(item => { item.addEventListener('click', () => { document.querySelectorAll('.nav-item').forEach(nav => nav.classList.remove('active')); item.classList.add('active'); currentPage = item.getAttribute('data-page'); if (currentPage === 'profile') showProfilePage(); else filterAndRender(); }); }); }

// ================= AUTH =================
function setupAuth() {
    document.getElementById('googleLoginBtn').onclick = async () => { try { const result = await auth.signInWithPopup(provider); currentUser = result.user; await usersRef.doc(currentUser.uid).set({ email: currentUser.email, displayName: currentUser.displayName, photoURL: currentUser.photoURL }, { merge: true }); closeLoginModal(); loadPosts(); } catch (e) { alert("Кирүү мүмкүн болбой калды"); } };
...
function showStatistics() { const totalLikes = allPosts.reduce((a, b) => a + (b.likes?.length || 0), 0); const totalSaves = allPosts.reduce((a, b) => a + (b.saves?.length || 0), 0); const totalComments = allPosts.reduce((a, b) => a + (b.comments?.length || 0), 0); showModal(`<div>Постлар: ${allPosts.length}<br>Жами лайк: ${totalLikes}<br>Жами сақланган: ${totalSaves}<br>Жами изоҳлар: ${totalComments}</div>`, "Статистика"); }
...
// ================= PROFIL SAHIFASI (Like, Save, Commented + Emoji + Base64) =================
async function showProfilePage() {
    if (!currentUser) {
        postsContainer.innerHTML = `<div class="post-card" style="text-align:center"><i class="fas fa-user-circle" style="font-size:4rem"></i><h3>${translations[currentLang].loginRequired}</h3><p>${translations[currentLang].loginDesc}</p><button class="btn-gradient" onclick="openLoginModal()"><i class="fab fa-google"></i> ${translations[currentLang].loginWithGoogle}</button></div>`;
        return;
    ...
    // Avatar / Emoji / Rasm ko‘rinishi
    let avatarHtml = '';
    if (profileEmoji) {
        avatarHtml = `<div style="font-size: 5rem; line-height: 1; margin-bottom: 10px;">${profileEmoji}</div>`;
    } else if (photoBase64) {
        avatarHtml = `<img src="${photoBase64}" class="profile-avatar">`;
    } else if (photoURL) {
        avatarHtml = `<img src="${photoURL}" class="profile-avatar">`;
    } else {
        const initials = (displayName || 'U').split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
        avatarHtml = `<div style="width:80px; height:80px; background: #4facfe; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 2rem; font-weight: bold; margin: 0 auto 10px auto;">${initials}</div>`;
    }

    // Like, save, commented postlar
    const likedPosts = allPosts.filter(p => p.likes?.includes(currentUser.uid));
    const savedPosts = allPosts.filter(p => p.saves?.includes(currentUser.uid));
    const commentedPosts = allPosts.filter(p => p.comments?.some(c => c.userId === currentUser.uid));
    ...
    let html = `
        <div class="post-card" style="text-align:center">
            ...
            <p>${escapeHtml(currentUser.email)}</p>
            <div class="profile-name-edit">
                <input type="text" id="editDisplayName" value="${escapeHtml(displayName)}" placeholder="Толук аты-жөн">
                <button onclick="updateProfileName()"><i class="fas fa-save"></i> Исмни оңдош</button>
            </div>
            <div style="margin-top: 10px; display: flex; gap: 8px; flex-wrap: wrap; justify-content: center;">
                <input type="file" id="profilePhotoInput" accept="image/*" style="display:none">
                <button class="btn-gradient" onclick="document.getElementById('profilePhotoInput').click()" style="background:#555; width:auto; padding:8px 16px"><i class="fas fa-camera"></i> Расм юклэш</button>
                <button class="btn-gradient" onclick="showEmojiPicker()" style="background:#f39c12; width:auto; padding:8px 16px"><i class="fas fa-smile"></i> Emoji танлаш</button>
                <button class="btn-gradient" onclick="openAdminModal()" style="margin:10px 0">➕ Пост қўшиш</button>
                <button class="btn-gradient" onclick="logout()" style="background:#555">🚪 Чыгуу</button>
            </div>
            <div style="margin-top:20px">
                <button class="btn-gradient" onclick="showBusinessTools()" style="background: linear-gradient(135deg, #f39c12, #e67e22)"><i class="fas fa-chart-line"></i> Бизнес қуроллары</button>
            </div>
            <div class="profile-liked-section"><h4><i class="fas fa-heart"></i> ${translations[currentLang].likes} (${likedPosts.length})</h4>${renderMiniPosts(likedPosts)}</div>
            <div class="profile-saved-section"><h4><i class="fas fa-bookmark"></i> ${translations[currentLang].saves} (${savedPosts.length})</h4>${renderMiniPosts(savedPosts)}</div>
            <div class="profile-commented-section"><h4><i class="fas fa-comment-dots"></i> ${translations[currentLang].comments} (${commentedPosts.length})</h4>${renderMiniPosts(commentedPosts)}</div>
    ...
    `;
    postsContainer.innerHTML = html;
    document.getElementById('profilePhotoInput')?.addEventListener('change', updateProfilePhotoBase64);
}